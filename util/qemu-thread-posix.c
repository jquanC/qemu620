...
/**
 * 
 * @param mode:根据 mode 参数的值，该线程要么是分离的，要么是可连接的。
*/
void qemu_thread_create(QemuThread *thread, const char *name,
                       void *(*start_routine)(void*),
                       void *arg, int mode)
{
    sigset_t set, oldset;//定义了两个 POSIX 信号集类型的变量
    int err;
    pthread_attr_t attr;//定义了一个线程属性变量
    QemuThreadArgs *qemu_thread_args; //是一个结构体指针，用于传递线程的参数。它会被动态分配内存，并将函数参数拷贝到相应的结构体成员中
    ...
    //最关键的一步，函数创建了一个新的线程并将其添加到调用进程的进程空间中。
    //qemu_thread_start 是线程的起始函数，qemu_thread_args 是传递给线程的参数
