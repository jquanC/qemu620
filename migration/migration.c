...
{
    // 在计算机领域中，"opaque"通常指代“不透明的”或“不可见的”，在代码中经常用作指向某个不透明类型的指针。
    // 它表示对于外部使用者来说，具体的实现是不可见的。这种模式通常用于隐藏具体的实现细节，提供一种封装的机制，
    // 防止外部直接访问内部实现，从而提高代码的安全性和可维护性。
    MigrationState *s = opaque;
    ...
    while (migration_is_active(s)) {
        if (urgent || !qemu_file_rate_limit(s->to_dst_file)) {
            //进入qemu_savevm_state_pending，qemu_savevm_state_iterate, migration_completion(s){qemu_savevm_state_complete_precopy , qemu_savevm_state_complete_postcopy}
